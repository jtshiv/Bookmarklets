			...
			for (let i = 0; i < $('div[role="listitem"]').length; i++) {
				if ($($('div[role="listitem"]')[i]).find('div[data-legacy-message-id]')[0]) {
					if (!$($('div[role="listitem"]')[i]).find('[data-downloadurl]')[0]) {
						dragdownload.setAttribute("data-downloadurl", "application/octet-stream:" + $('[role="main"]').find('[data-legacy-thread-id]')[0].innerText.replace(/[|&;:$%@"<>()+,]/g, "") + ".eml:https://mail.google.com/mail/u/0?view=att&th=" + $($('div[role="listitem"]')[i]).find('div[data-legacy-message-id]')[0].getAttribute("data-legacy-message-id") + "&disp=comp&safe=1&zw");
